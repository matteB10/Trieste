...
namespace trieste
{
  ...
  private:
    ...
    {
      auto ast = wf.gen(generators_, context.current_seed, max_depth_);
             ...
             context.retries < max_retries_)
      {
        ...
        ast = wf.gen(generators_, context.current_seed, max_depth_);
        hash = ast->hash();
        context.retries++;
      }
        ...
        if (result == RunResult::FAIL)
        {
          ...
          if (!logging::Trace::active())
          {
            ...
            err << "============" << std::endl
                ...
                << ", seed: " << seed_context.current_seed << std::endl
                << "------------" << std::endl
                << prev.gen(generators_, seed_context.current_seed, max_depth_)
                << "------------" << std::endl
                << new_ast;
          }
