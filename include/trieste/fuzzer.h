...
namespace trieste
{
  ...
  private:
    ...
    /// @return The result of running the pass, including the new AST, success
    /// status, and number of changes.
    PassResult run_pass(
      Node& ast,
      const Pass& pass,
    ...
    /// @return the statistics for the pass test
    PassStats
    test_pass_with_survivors(Pass& pass, std::vector<Survivor>& survivors)
    {
      PassStats pass_stats;
      for (auto& survivor : survivors)
      {
        logging::Trace() << "============" << std::endl
                         << "Pass: " << pass->name() << ", survivor from seed "
                         << survivor.original_seed << std::endl
                         << "------------" << std::endl
        ...
        if (result == RunResult::FAIL)
        {
          ...
          err << "============" << std::endl
              << "Failed pass: " << pass->name() << ", survivor from seed "
              << survivor.original_seed << std::endl;
          if (failfast_)
            break;
        }
    ...
    int test()
    {
      ...
      for (size_t i = start_index_; i <= end_index_; i++)
      {
        ...
        if (test_sequence_)
        {
          ...
          if (size_stats_)
          {
            sequence_stats.error_sizes_.insert(
              sequence_stats.error_sizes_.end(),
              pass_stats.error_sizes.begin(),
              pass_stats.error_sizes.end());
            sequence_stats.error_heights_.insert(
              sequence_stats.error_heights_.end(),
              pass_stats.error_heights.begin(),
              pass_stats.error_heights.end());
          }
        }
